...
DACUpdateSample:
	...
	tst.b	(UniDACMute_Count).w
	bne.s	locret_71CAA
	tst.b	SMPS_Track.MuteChannel(a5)
	bne.s	locret_71CAA
...
.nopalmode:
	...
	moveq	#0,d7
	move.w	(MusicVoice_Count).w,d7
	beq.s	.loadbank
	add.w	d7,d7
	add.w	d7,d7
	lea	SoundDriver_Banks-4(pc),a1
	movea.l	(a1,d7.w),a1
	move.w	(MusicVoice_Index).w,d7
	mulu.w	#$19,d7
	adda.w	d7,a1
	move.l	a1,d2
	bra.s	.skipbank
...
.loadbank:
	move.l	(a4),d2			; Load voice pointer

.skipbank:
	move.b	4+4+1(a4),d0		; Load tempo
...
; Load pointer
	...
	move.b	(a2)+,d0
	beq.s	.skipdata
	addq.w	#4,a4			; skip DataPointer
	add.w	d0,d0
	add.w	d0,d0
	lea	MusicIndex-4(pc),a5
	movea.l	(a5,d0.w),a5
	cmpa.l	#Clone_Driver_RAM+SMPS_RAM.v_music_dac_track,a1
	bne.s	.fmpointer
...
.setdata:
	move.l	d0,SMPS_Track.DataPointer(a1)		; Load DAC/FM pointer
	move.b	(a4)+,d0
	add.b	(a2)+,d0
	move.b	d0,SMPS_Track.Transpose(a1)	; Load FM channel modifier
	move.b	(a4)+,d0
	add.b	(a2)+,d0
	add.b	(UniFMVol_Count).w,d0
	move.b	d0,SMPS_Track.Volume(a1)	; Load FM channel modifier
	move.l	d2,SMPS_Track.VoicePtr(a1)	; Load voice pointer
	move.b	(a2)+,SMPS_Track.MuteChannel(a1)	; Load DAC/FM channel mute
	adda.w	d6,a1
...
.setpsgdata:
	move.l	d0,SMPS_Track.DataPointer(a1)		; Load PSG channel pointer
	move.b	(a4)+,d0
	add.b	(a2)+,d0
	move.b	d0,SMPS_Track.Transpose(a1)	; Load PSG modifier
	move.b	(a4)+,d0
	add.b	(a2)+,d0
	add.b	(UniPSGVol_Count).w,d0
	move.b	d0,SMPS_Track.Volume(a1)	; Load PSG modifier
	...
	move.b	(a2)+,SMPS_Track.MuteChannel(a1)	; Load PSG channel mute
	adda.w	d6,a1
